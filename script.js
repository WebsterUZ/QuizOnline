...
function shuffleOptions(question) {
    ...
    // Rearrange options and update correctAnswer
    const shuffledOptions = indices.map(i => question.options[i]);
    const newCorrect = indices.indexOf(question.correctAnswer);
...
function displayQuestion() {
    ...
    options.forEach(optionLabel => {
        optionLabel.addEventListener('click', () => {
            if (!quizContainer.dataset.answered) {
                ...
                // To'g'ri javobni har doim yashil qilamiz
                options[correctAnswerIndex].classList.add('list-group-item-success');

                // Agar noto'g'ri tanlansa, uni qizil qilamiz
                if (selectedAnswerIndex !== correctAnswerIndex) {
                    options[selectedAnswerIndex].classList.add('list-group-item-danger');
                } else {
                    score++;
                }
            }
...
function showResults() {
    ...
    let reviewHtml = `
        <div class="text-center">
            ...
            <p class="lead">Sizning natijangiz: ${score} / ${currentQuestions.length}</p>
